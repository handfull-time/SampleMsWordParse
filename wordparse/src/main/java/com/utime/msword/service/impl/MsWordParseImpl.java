package com.utime.msword.service.impl;
...
@Service
class MsWordParseImpl implements MsWordParse {

    @Override
    public List<DocElement> Parser(InputStream is) throws IOException {
	 ...
	 * @param para
	 */
	private void procXWPFParagraph(List<DocElement> docList, DocNumLevelListMgr numLevelListMgr, XWPFParagraph para ) {
	            ...
	            if (run instanceof XWPFRun) {
	                ...
	                if( pictures != null && pictures.size() > 0 ) {
	                	...
	                	if( isFirst ) {
	                		...
	                		txt.setText( this.getFrontNum(numLevelListMgr, para) );
	                		isFirst = false;
	                	}
    ...
    private void procBodyElement(List<DocElement> docList, Iterator<IBodyElement> itBody ) {
		...
		while (itBody.hasNext()) {
			...
			case PARAGRAPH:{
				...
				this.procXWPFParagraph( docList, numLevelListMgr, paragraph );
				break;
			}
			case TABLE:{
	 ...
	 * @return
	 */
	private String getFrontNum( DocNumLevelListMgr numLevelListMgr, XWPFParagraph para ) {
		String result = "";
